...
def import_old_reports(export_file):
    """
    Imports old reports (i.e. those done by Nick Ferris, Paul Beech and AXIS
    Reporting from a TSV file generated by the 'Export study reports'
    script in the FileMaker DB. Cannot use the same mechanism as the filemaker
    sync method as the report files are stored in "container" fields that are
    not exported when you export the table
    ...
    for row in tqdm(rows[1:]):
        ...
        if d['MrReport']:
            if 'MSH' in d['MrReport']:
                reporter = axis
            else:
                reporter = nick_ferris
            db.session.add(Report(  # noqa pylint: disable=no-member
                img_session.id, reporter.id, '', NOT_RECORDED,
                [], MRI, date=img_session.scan_date, dummy=True))
        if d['PetReport']:
            db.session.add(Report(  # noqa pylint: disable=no-member
                img_session.id, paul_beech.id, '', NOT_RECORDED,
                [], PET, date=img_session.scan_date, dummy=True))  # noqa pylint: disable=no-member
        db.session.commit()  # pylint: disable=no-member
...
#                       password=app.config['SOURCE_XNAT_PASSWORD']) as m_xnat:
#         with open(export_file) as f:
...
#                     # Add dummy reports if existing report was stored in FM
#                     if row['MrReport'] is not None and row[
#                             'MrReport'].strip():
    ...
    if args.action == 'init':
        ...
        try:
            ...
            print("Successfully initialised database at {}"
                  .format(db_path))
    elif args.action == 'import-reports':
        ...
        import_old_reports(import_args.export_file)
    else:
