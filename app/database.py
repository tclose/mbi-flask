...
def import_old_reports(export_file):
    """
    Imports data from a TSV file generated by the 'Export study reports'
    script in the FileMaker DB. Includes previously submitted reports which are
    not exported when you just dump the tables to file as they are stored in
    "container" fields
    ...
    with open(export_file) as f:
        ...
        for row in tqdm(rows):
            img_session = ImgSession.query.filter_by(id=row['SessionID']).one()
